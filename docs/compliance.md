...
| Identità del titolare in ogni pagina | `layout.tsx` + `src/lib/legal.ts` | fatto |

**Titolare:** Salvatore Campagnese, ditta individuale, Via Fratelli Cervi 22,
40024 Castel San Pietro Terme (BO) — P. IVA `04254271200` (checksum verificato,
codice ufficio 120 = Bologna, coerente col CAP). Contatto e reclami:
`info@altuodiocipensoio.com`.

Essendo una ditta individuale, il titolare del trattamento è una **persona
fisica che risponde in proprio**: non c'è schermo societario fra le sanzioni e
il patrimonio personale. È la ragione per cui i punti qui sotto meritano
attenzione più di quanta ne meriterebbero in una S.r.l.

## Cosa manca, in ordine di urgenza

### 1. Due campi anagrafici ancora vuoti

In `src/lib/legal.ts`:

- **`taxCode`** — il codice fiscale personale a 16 caratteri. Per una ditta
  individuale è diverso dalla partita IVA e va indicato.
- **`pec`** — domicilio digitale. Obbligatorio per le imprese iscritte al
  Registro Imprese; se sei iscritto, va aggiunto anche il numero **REA**.

Non bloccano la pubblicazione (l'avviso rosso è sparito), ma completano
l'identificazione richiesta dagli obblighi informativi.
...
### 4-bis. Verificare se serve un DPO

L'informativa dichiara oggi che non è stato nominato un responsabile della
protezione dei dati, il che è vero. L'art. 37 par. 1 lett. c lo impone però a
chi tratta **su larga scala** dati dell'art. 9 — e questo servizio tratta
convinzioni religiose e dati sulla salute come attività principale, non
accessoria.

Se il volume cresce, la nomina può diventare obbligatoria. È una valutazione da
fare col consulente, e l'informativa va aggiornata di conseguenza.
